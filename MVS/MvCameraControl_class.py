...
def check_sys_and_update_dll():
    ...
    if currentsystem == 'Windows':
        # Get the directory of the current file (MVS folder)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Path to the DLL - relative to this file's location
        MvCamCtrldllPath = os.path.join(current_dir, "Win64_x64", "MvCameraControl.dll")
        
        # Get the directory containing the DLL
        dll_directory = os.path.dirname(MvCamCtrldllPath)
        
        # Add the DLL directory to the DLL search path (Python 3.8+)
        # This allows Windows to find dependent DLLs
        if hasattr(os, 'add_dll_directory'):
            os.add_dll_directory(dll_directory)
        
        # Also add to system PATH as fallback
        os.environ['PATH'] = dll_directory + os.pathsep + os.environ['PATH']
        
        # Verify the DLL exists
        if not os.path.exists(MvCamCtrldllPath):
            raise FileNotFoundError(f"DLL not found: {MvCamCtrldllPath}")
        
        print(f"Loading DLL from: {MvCamCtrldllPath}")
        
        try:
            if "winmode" in ctypes.WinDLL.__init__.__code__.co_varnames:
                MvCamCtrldll = ctypes.WinDLL(MvCamCtrldllPath, winmode=0)
            else:
                MvCamCtrldll = ctypes.WinDLL(MvCamCtrldllPath)
            print("DLL loaded successfully!")
        except Exception as e:
            print(f"Error loading DLL: {e}")
            print(f"\nCheck that all dependent DLLs are in: {dll_directory}")
            raise
